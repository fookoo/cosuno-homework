...
import { Checkbox, FormControlLabel, Grid, Input } from '@material-ui/core'
...
export const FilterPanel: React.FC<IFilterPanelProps> = ({ onNameQuery, onFeatureQuery }) => {
  ...
  const [queryTerm] = useDebounce(query, 200)
  ...
  return (
    <Grid container direction={'row'} justifyContent={'space-between'}>
      <Grid item>
        <Input placeholder="Search by name" type="search" value={query} onChange={handleChange} />
      </Grid>
      <Grid item>
        <FormControlLabel
          control={
